...
export function StepSign({ state, setState, onBack }: StepProps) {
  ...
  async function handleSign() {
    ...
    try {
      ...
      await supabase
        .from("profiles")
        .update({ active_run_id: run.id } as never)
        .eq("id", user.id);
      ...
      if (state.weightKg && state.weightKg > 0) {
  ...
  return (
    <div>
      ...
      <p className="font-[var(--font-display)] italic text-[21px] leading-[1.45] text-ink-2 mt-[22px] max-w-[640px]">
        Everything you declared, rendered as a formal document. A real contract gets read before it
        gets signed.
      </p>
      ...
      <div className="mt-8 border border-hair rounded-[14px] overflow-hidden bg-card shadow-[0_24px_48px_-28px_rgba(20,15,10,0.2)]">
        ...
        <div className="p-[28px_36px] border-b border-hair-2">
          ...
          <div className="bg-ink text-bone rounded-[10px] p-[24px_28px]">
            <div className="font-[var(--font-tactical)] text-[10px] tracking-[0.16em] uppercase text-ink-4 mb-1.5">
              SIGNED BY
            </div>
            <div className="font-[var(--font-tactical)] text-[22px] font-semibold">
              @{state.displayName || "player"}
            </div>
            <div className="mt-[22px] pt-[22px] border-t border-[#2a2520]">
              <div className="font-[var(--font-tactical)] text-[10px] tracking-[0.16em] uppercase text-ink-4 mb-2.5">
                YOUR ANCHOR · WRITTEN ON STEP 02
              </div>
            ...
            {[
              {
                ...
                name: "Body",
                rows: [
                  ["Cadence", `${state.bodyDays.length}× / week`],
                  ["Min", `${state.bodyMinutes} min`],
                  ["Days", state.bodyDays.map((d) => DAY_NAMES[d]).join(" · ")],
                ],
              },
              {
                n: "II",
              ...
              },
              {
                ...
                name: "Craft",
                rows: [
                  ["Cadence", `${state.craftDays.length}× / week`],
                  ...state.craftSubtasks.map((s, i) => [
                    `Subtask ${String.fromCharCode(65 + i)}`,
              ...
              <div key={core.n} className="border border-hair rounded-[10px] p-5 bg-bone relative">
                <span className="absolute top-4 right-4 font-[var(--font-tactical)] text-[9px] tracking-[0.14em] uppercase text-ember flex items-center gap-1.5">
                  LOCKED
                </span>
                <span className="font-[var(--font-display)] italic text-[36px] text-ember leading-none">
                  {core.n}
                </span>
                <h5 className="font-[var(--font-display)] italic text-[24px] mt-1.5 mb-3">
                  {core.name}
                </h5>
                {core.rows.map(([k, v]) => (
                  <div
                    key={k}
                    className="flex justify-between py-1.5 border-t border-hair-2 text-[12.5px]"
                  >
                    <span className="font-[var(--font-tactical)] text-[10px] tracking-[0.12em] uppercase text-ink-3">
                      {k}
                    </span>
          ...
          <div className="grid grid-cols-7 gap-2.5 mt-2.5">
            {[
              ...
              <div key={d.n} className="text-center p-3.5 bg-bone border border-hair rounded-[8px]">
                <div className="font-[var(--font-tactical)] text-[10px] tracking-[0.14em] uppercase text-ink-3 mb-2">
                  {d.label}
                </div>
        ...
        <div className="bg-ink text-bone p-12 text-center relative overflow-hidden">
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse 50% 100% at 50% 0%, rgba(232,162,107,0.08), transparent 70%)",
            }}
          />

          {/* Seal */}
          <div
            className="relative z-[2] w-[88px] h-[88px] rounded-full border-[1.5px] border-ember mx-auto flex items-center justify-center"
          ...
          >
            ...
            <div
              className="absolute -inset-2.5 rounded-full border border-dashed border-ember opacity-55"
              style={{ animation: "rotate 30s linear infinite" }}
            />
            <div
              className="absolute -inset-[22px] rounded-full border border-dotted border-ember opacity-25"
          ...
          {state.anchorText && (
            <p className="relative z-[2] mt-6 text-[13px] text-ink-4 max-w-[560px] mx-auto leading-[1.55] italic">
              Echoing your anchor: &ldquo;{state.anchorText.slice(0, 120)}
              {state.anchorText.length > 120 ? "..." : ""}&rdquo;
            </p>
          )}
          ...
          >
            ...
            <span className="transition-transform hover:translate-x-1.5">→</span>
          </button>
